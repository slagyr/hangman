...
class TextUI < HangmanAbstractUI
  def initialize
    @dialog = []
    out 'ui initialized'
  end
  ...
  def new_game(player, guesses_per_game)
    ...
    out "Player #{player.name} has started a new game"
    draw_page
  end

  def fail(reason)
    out "Player #{@player.name} has been disqualified because: #{reason}"
    draw_page
  end
  
  def guessed(letter)
    out "Player #{@player.name} guessed #{letter}"
